...
function writeCodexConfigFile({ agentsRoot, codex }) {
  const runtimeDir = path.join(agentsRoot, ".runtime");
  ensureDir(runtimeDir);
  const target = path.join(runtimeDir, "codex.generated.toml");
...
function getQueues(requirementsRoot) {
  return {
    ...
    needToCheck: path.join(requirementsRoot, "need-to-check"),
...
function normalizeFlow(value, fallback) {
  ...
  return ["standard", "bulk", "fast"].includes(normalized) ? normalized : fallback;
}
...
function loadRuntimeConfig(agentsRoot) {
  ...
  const codexConfigPath = writeCodexConfigFile({ agentsRoot, codex });
