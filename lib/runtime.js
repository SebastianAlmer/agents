...
function loadRuntimeConfig(agentsRoot) {
  ...
  const po = {
    defaultMode: normalizeMode(
      config.po && config.po.default_mode,
      DEFAULTS.po.default_mode,
      ["intake", "vision"]
    ),
    visionMaxCycles: Math.max(1, toInteger(
      config.po && config.po.vision_max_cycles,
      DEFAULTS.po.vision_max_cycles
    )),
    visionMaxRequirements: Math.max(1, toInteger(
      config.po && config.po.vision_max_requirements,
      DEFAULTS.po.vision_max_requirements
    )),
    visionStableCycles: Math.max(1, toInteger(
      config.po && config.po.vision_stable_cycles,
