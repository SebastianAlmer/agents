...
const DEFAULTS = {
  ...
  bundle_flow: {
    ...
    id_pad: 4,
    max_ready_ahead: 1,
    enforce_single_active_bundle: true,
    carryover_target_queue: "refinement",
    branch_prefix: "rb",
    allow_cross_bundle_moves: false,
  },
  delivery_runner: {
    default_mode: "full",
    agent_timeout_seconds: 1800,
    no_output_timeout_seconds: 600,
    max_paused_cycles_per_item: 2,
  ...
  retry_policy: {
    ...
    uat_retry_max: 1,
    deploy_retry_max: 0,
    po_intake_retry_max: 1,
    po_vision_retry_max: 1,
    maint_retry_max: 0,
    qa_post_retry_max: 0,
    ux_final_retry_max: 0,
    sec_final_retry_max: 0,
    uat_full_retry_max: 0,
  },
  loop_policy: {
    loop_window_cycles: 20,
    loop_threshold: 3,
    max_total_attempts_per_req: 5,
    escalate_business_loop_to_human_decision: true,
  },
  delivery_quality: {
    strict_gate: true,
  ...
  thread_recovery: {
    rotate_after_runs: 0,
    reset_on_compact_or_model_error: true,
  },
  qa: {
    mandatory_checks: [],
...
const MODEL_FALLBACK_TRIGGER_REASONS = [
  "usage_limit",
  "rate_limit",
  "too_many_requests",
  "retry_later",
...
function normalizeTriggerReasons(value, fallback) {
  const source = Array.isArray(value) ? value : fallback;
  const allowed = new Set(MODEL_FALLBACK_TRIGGER_REASONS);
  const out = [];
  ...
  return out.length > 0 ? out : [...MODEL_FALLBACK_TRIGGER_REASONS];
}
...
function loadRuntimeConfig(agentsRoot) {
  ...
  const productVisionDir = productVisionRaw.trim()
    ...
    : (legacyProductVisionDir || detectProductVisionDir(docsDir));
  ...
  const bundleFlowRaw = merged.bundle_flow && typeof merged.bundle_flow === "object"
    ? merged.bundle_flow
    : {};
  const carryoverTargetRaw = String(
    bundleFlowRaw.carryover_target_queue || DEFAULTS.bundle_flow.carryover_target_queue
  ).trim();
  const bundleFlow = {
    enabled: toBoolean(
      bundleFlowRaw.enabled,
      DEFAULTS.bundle_flow.enabled
    ),
    idPrefix: String(bundleFlowRaw.id_prefix || DEFAULTS.bundle_flow.id_prefix).trim() || "B",
    idPad: Math.max(1, toInteger(bundleFlowRaw.id_pad, DEFAULTS.bundle_flow.id_pad)),
    maxReadyAhead: Math.max(1, toInteger(bundleFlowRaw.max_ready_ahead, DEFAULTS.bundle_flow.max_ready_ahead)),
    enforceSingleActiveBundle: toBoolean(
      bundleFlowRaw.enforce_single_active_bundle,
      DEFAULTS.bundle_flow.enforce_single_active_bundle
    ),
    carryoverTargetQueue: [
      "refinement",
      "backlog",
      "toClarify",
      "humanInput",
      "humanDecisionNeeded",
    ].includes(carryoverTargetRaw)
      ? carryoverTargetRaw
      : DEFAULTS.bundle_flow.carryover_target_queue,
    branchPrefix: String(bundleFlowRaw.branch_prefix || DEFAULTS.bundle_flow.branch_prefix).trim() || "rb",
    allowCrossBundleMoves: toBoolean(
      bundleFlowRaw.allow_cross_bundle_moves,
      DEFAULTS.bundle_flow.allow_cross_bundle_moves
    ),
  };
  ...
  const deliveryRunner = {
    ...
    agentTimeoutSeconds: Math.max(
      0,
      toInteger(
        merged.delivery_runner && merged.delivery_runner.agent_timeout_seconds,
        DEFAULTS.delivery_runner.agent_timeout_seconds
      )
    ),
    noOutputTimeoutSeconds: Math.max(
      0,
      toInteger(
        merged.delivery_runner && merged.delivery_runner.no_output_timeout_seconds,
        DEFAULTS.delivery_runner.no_output_timeout_seconds
      )
    ),
    maxPausedCyclesPerItem: Math.max(
      1,
      toInteger(
        merged.delivery_runner && merged.delivery_runner.max_paused_cycles_per_item,
        DEFAULTS.delivery_runner.max_paused_cycles_per_item
      )
    ),
    workspaceBranchesEnabled: toBoolean(
      merged.delivery_runner && merged.delivery_runner.workspace_branches_enabled,
      DEFAULTS.delivery_runner.workspace_branches_enabled
    ),
  };

  const pauseRaw = merged.pause_policy && typeof merged.pause_policy === "object"
    ? merged.pause_policy
    : {};
  const pausePolicy = {
    probeOnStartup: toBoolean(
      pauseRaw.probe_on_startup,
      DEFAULTS.pause_policy.probe_on_startup
    ),
    probeCooldownSeconds: clamp(
      pauseRaw.probe_cooldown_seconds,
      10,
      86_400,
      DEFAULTS.pause_policy.probe_cooldown_seconds
    ),
    probeTimeoutSeconds: clamp(
      pauseRaw.probe_timeout_seconds,
      5,
      600,
      DEFAULTS.pause_policy.probe_timeout_seconds
    ),
  };

  const retryRaw = merged.retry_policy && typeof merged.retry_policy === "object"
    ? merged.retry_policy
    : {};
  const retryPolicy = {
    archRetryMax: Math.max(0, toInteger(retryRaw.arch_retry_max, DEFAULTS.retry_policy.arch_retry_max)),
    uxRetryMax: Math.max(0, toInteger(retryRaw.ux_retry_max, DEFAULTS.retry_policy.ux_retry_max)),
    secRetryMax: Math.max(0, toInteger(retryRaw.sec_retry_max, DEFAULTS.retry_policy.sec_retry_max)),
    qaRetryMax: Math.max(0, toInteger(retryRaw.qa_retry_max, DEFAULTS.retry_policy.qa_retry_max)),
    uatRetryMax: Math.max(0, toInteger(retryRaw.uat_retry_max, DEFAULTS.retry_policy.uat_retry_max)),
    deployRetryMax: Math.max(0, toInteger(retryRaw.deploy_retry_max, DEFAULTS.retry_policy.deploy_retry_max)),
    poIntakeRetryMax: Math.max(0, toInteger(retryRaw.po_intake_retry_max, DEFAULTS.retry_policy.po_intake_retry_max)),
    poVisionRetryMax: Math.max(0, toInteger(retryRaw.po_vision_retry_max, DEFAULTS.retry_policy.po_vision_retry_max)),
    maintRetryMax: Math.max(0, toInteger(retryRaw.maint_retry_max, DEFAULTS.retry_policy.maint_retry_max)),
    qaPostRetryMax: Math.max(0, toInteger(retryRaw.qa_post_retry_max, DEFAULTS.retry_policy.qa_post_retry_max)),
    uxFinalRetryMax: Math.max(0, toInteger(retryRaw.ux_final_retry_max, DEFAULTS.retry_policy.ux_final_retry_max)),
    secFinalRetryMax: Math.max(0, toInteger(retryRaw.sec_final_retry_max, DEFAULTS.retry_policy.sec_final_retry_max)),
    uatFullRetryMax: Math.max(0, toInteger(retryRaw.uat_full_retry_max, DEFAULTS.retry_policy.uat_full_retry_max)),
  };

  const loopRaw = merged.loop_policy && typeof merged.loop_policy === "object"
    ? merged.loop_policy
    : {};
  const loopPolicy = {
    loopWindowCycles: Math.max(1, toInteger(loopRaw.loop_window_cycles, DEFAULTS.loop_policy.loop_window_cycles)),
    loopThreshold: Math.max(2, toInteger(loopRaw.loop_threshold, DEFAULTS.loop_policy.loop_threshold)),
    maxTotalAttemptsPerReq: Math.max(1, toInteger(loopRaw.max_total_attempts_per_req, DEFAULTS.loop_policy.max_total_attempts_per_req)),
    escalateBusinessLoopToHumanDecision: toBoolean(
      loopRaw.escalate_business_loop_to_human_decision,
      DEFAULTS.loop_policy.escalate_business_loop_to_human_decision
    ),
  };
  ...
  const deliveryQuality = {
    ...
    uatEnabled: toBoolean(
      merged.delivery_quality && merged.delivery_quality.uat_enabled,
      DEFAULTS.delivery_quality.uat_enabled
    ),
  ...
  const releaseRaw = merged.release_automation && typeof merged.release_automation === "object"
    ? merged.release_automation
    : {};
  const releaseMergeMode = String(
    releaseRaw.merge_mode || DEFAULTS.release_automation.merge_mode || "ff-only"
  ).toLowerCase();
  const releaseAutomation = {
    enabled: toBoolean(
      releaseRaw.enabled,
      DEFAULTS.release_automation.enabled
    ),
    baseBranch: String(
      releaseRaw.base_branch || DEFAULTS.release_automation.base_branch || "dev"
    ).trim() || "dev",
    remote: String(
      releaseRaw.remote || DEFAULTS.release_automation.remote || "origin"
    ).trim() || "origin",
    branchPrefix: String(
      releaseRaw.branch_prefix || DEFAULTS.release_automation.branch_prefix || "rb"
    ).trim() || "rb",
    versionScope: String(
      releaseRaw.version_scope || DEFAULTS.release_automation.version_scope || "root"
    ).trim().toLowerCase() || "root",
    versionCommand: String(
      releaseRaw.version_command || DEFAULTS.release_automation.version_command || "npm version patch --no-git-tag-version"
    ).trim() || "npm version patch --no-git-tag-version",
    mergeMode: releaseMergeMode === "ff-only" ? "ff-only" : "ff-only",
    tagEnabled: toBoolean(
      releaseRaw.tag_enabled,
      DEFAULTS.release_automation.tag_enabled
    ),
    tagPrefix: String(
      releaseRaw.tag_prefix || DEFAULTS.release_automation.tag_prefix || "v"
    ).trim() || "v",
    autoResolveConflicts: toBoolean(
      releaseRaw.auto_resolve_conflicts,
      DEFAULTS.release_automation.auto_resolve_conflicts
    ),
    maxConflictFixAttempts: Math.max(
      0,
      toInteger(
        releaseRaw.max_conflict_fix_attempts,
        DEFAULTS.release_automation.max_conflict_fix_attempts
      )
    ),
    allowReleaseWithHumanDecisionNeeded: toBoolean(
      releaseRaw.allow_release_with_human_decision_needed,
      DEFAULTS.release_automation.allow_release_with_human_decision_needed
    ),
  };

  const releaseHistoryRaw = merged.release_history && typeof merged.release_history === "object"
    ? merged.release_history
    : {};
  const releaseHistoryFileRaw = String(
    releaseHistoryRaw.file || DEFAULTS.release_history.file || "docs/release-history.md"
  ).trim();
  const releaseHistorySourceRaw = String(
    releaseHistoryRaw.source_file || DEFAULTS.release_history.source_file || ""
  ).trim();
  const releaseHistoryAgent = String(
    releaseHistoryRaw.agent || DEFAULTS.release_history.agent || "deploy"
  ).trim().toLowerCase();
  const releaseHistory = {
    enabled: toBoolean(
      releaseHistoryRaw.enabled,
      DEFAULTS.release_history.enabled
    ),
    required: toBoolean(
      releaseHistoryRaw.required,
      DEFAULTS.release_history.required
    ),
    file: releaseHistoryFileRaw
      ? resolvePathValue(releaseHistoryFileRaw, repoRoot)
      : path.join(docsDir, "release-history.md"),
    sourceFile: releaseHistorySourceRaw
      ? resolvePathValue(releaseHistorySourceRaw, repoRoot)
      : "",
    agent: releaseHistoryAgent === "deploy" ? "deploy" : "deploy",
  };
  ...
  const threadRecovery = {
    rotateAfterRuns: Math.max(
      0,
      toInteger(
        merged.thread_recovery && merged.thread_recovery.rotate_after_runs,
        DEFAULTS.thread_recovery.rotate_after_runs
      )
    ),
    resetOnCompactOrModelError: toBoolean(
      merged.thread_recovery && merged.thread_recovery.reset_on_compact_or_model_error,
      DEFAULTS.thread_recovery.reset_on_compact_or_model_error
    ),
  };
  ...
  const fallbackRaw = merged.model_fallback && typeof merged.model_fallback === "object"
    ? merged.model_fallback
    : {};
  const fallbackModelsRaw = fallbackRaw.models && typeof fallbackRaw.models === "object"
    ? fallbackRaw.models
    : {};
  const fallbackModels = resolveCodexModels({
    rawModels: fallbackModelsRaw,
    fallbackModel: normalizeModelName(fallbackModelsRaw.default || fallbackModelsRaw.all, ""),
  });
  const modelFallback = {
    maxAttemptsPerCodexRun: Math.max(
      0,
      toInteger(
        fallbackRaw.max_attempts_per_codex_run,
        DEFAULTS.model_fallback.max_attempts_per_codex_run
      )
    ),
    reuseThread: toBoolean(
      fallbackRaw.reuse_thread,
      DEFAULTS.model_fallback.reuse_thread
    ),
    triggerReasons: normalizeTriggerReasons(
      fallbackRaw.trigger_reasons,
      DEFAULTS.model_fallback.trigger_reasons
    ),
  ...
  const fallbackCodexConfigPathsByAgent = {};
  for (const [agent, model] of Object.entries(fallbackModels.byAgent)) {
    const fallbackModel = normalizeModelName(model, "");
    if (!fallbackModel) {
      continue;
    }
    fallbackCodexConfigPathsByAgent[agent] = writeCodexConfigFile({
      agentsRoot,
      codex: {
        ...codex,
        model: fallbackModel,
      ...
      fileName: `codex.generated.${agent.toLowerCase()}.fallback.toml`,
    });
  }
  ...
  const resolveAgentFallbackCodexConfigPath = (agentLabel) => {
    const normalized = normalizeAgentLabel(agentLabel);
    return fallbackCodexConfigPathsByAgent[normalized] || "";
  };
  ...
  return {
    ...
    fallbackCodexConfigPathsByAgent,
    resolveAgentCodexConfigPath,
    resolveAgentFallbackCodexConfigPath,
