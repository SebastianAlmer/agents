...
async function runDevAgent({ agentRoot, agentLabel, promptLabel, targetScope }) {
  ...
  console.log(`${agentLabel}: human-decision-needed dir ${clarifyDir}`);
  ...
  const stableContext = [
    ...
    "- If critical information is missing or contradictory, stop and route to human-decision-needed with concrete questions.",
  ...
  const configArgs = readConfigArgs(runtime.resolveAgentCodexConfigPath(agentLabel));
