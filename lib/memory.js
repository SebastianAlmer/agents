...
function defaultSharedTemplate() {
  return [
    ...
    "- ",
    "",
    "## Recent Work Context (last runs)",
    "- Path(s): <repo-relative paths>",
    "  Decision: <what was decided>",
    "  Outcome: <pass/fail/blocked + short reason>",
    "",
    "## Known Pitfalls",
    "- ",
    "",
...
function defaultAgentTemplate(agentLabel) {
  return [
    ...
    "## Recent Work Context (last runs)",
    "- Path(s): <repo-relative paths>",
    "  Decision: <implementation/routing decision>",
...
function buildMemoryUpdatePrompt(runtime, agentLabel, files) {
  ...
  return [
    ...
    "- Preserve short run context so next run knows where work happened.",
    ...
    "- For every new run note, include both touched path(s) and decision made.",
    "- Keep run notes short: path(s) + decision + outcome/check.",
    "- Keep only recent run context (drop stale run-by-run detail).",
    "- Keep durable rules separate from recent run context.",
