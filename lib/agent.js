...
function acquireThreadLock(lockPath, agentLabel) {
  ensureDir(path.dirname(lockPath));
  const lockData = {
    pid: process.pid,
    agent: String(agentLabel || "AGENT"),
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const owner = existing && existing.pid ? `pid=${existing.pid}` : "unknown owner";
      throw new Error(
        `${agentLabel || "AGENT"}: interactive thread lock active (${owner}); close other session or remove ${lockPath}`
      );
    }
  }
  throw new Error(`${agentLabel || "AGENT"}: failed to acquire thread lock`);
}

function releaseThreadLock(lockPath, fd) {
  if (typeof fd === "number") {
    try {
      fs.closeSync(fd);
    } catch {
...
function buildInteractiveArgs({ configArgs, threadId, prompt }) {
  const args = [];
  if (configArgs && configArgs.length) {
    args.push(...configArgs);
  }
...
async function runCodexInteractiveSession({
  repoRoot,
  configArgs,
  threadId,
  prompt,
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error(`${agentLabel || "AGENT"}: interactive mode requires a TTY terminal`);
  }

  const args = buildInteractiveArgs({ configArgs, threadId, prompt });
  const proc = spawn("codex", args, {
    cwd: repoRoot,
    stdio: "inherit",
  });
  ...
  if (exitCode !== 0) {
    const prefix = agentLabel || "AGENT";
    throw new Error(`${prefix}: codex interactive session failed with exit code ${exitCode}`);
  }
}
...
async function startInteractiveCodexAgent({
  ...
  const lockPath = `${threadFile}.lock`;
  const lockFd = acquireThreadLock(lockPath, agentLabel);
  try {
    if (!activeThreadId) {
      if (!bootstrapPrompt) {
        throw new Error(`${agentLabel || "AGENT"}: bootstrap prompt is required for first start`);
      }
      const bootstrapInput = [
        String(bootstrapPrompt || "").trim(),
        "",
        "Bootstrap mode:",
        "- Persist this context for future resumed sessions.",
        "- Do not edit files.",
        "- Do not run any shell commands.",
        "- Reply with exactly: THREAD_READY",
      ].join("\n");

      process.stdout.write(`${agentLabel}: first start, creating persistent thread\n`);
      const bootstrapResult = await runCodexExec({
        prompt: bootstrapInput,
        repoRoot,
        configArgs,
        threadId: "",
        threadFile,
      ...
      activeThreadId = bootstrapResult.threadId || "";
      if (!activeThreadId) {
        throw new Error(`${agentLabel}: failed to create persistent thread`);
      }
      writeThreadId(threadFile, activeThreadId);
      process.stdout.write(`${agentLabel}: thread saved ${activeThreadId}\n`);
    } else {
      process.stdout.write(`${agentLabel}: resuming thread ${activeThreadId}\n`);
    }

    await runCodexInteractiveSession({
      repoRoot,
      configArgs,
    ...
    releaseThreadLock(lockPath, lockFd);
  }
}
